...
module.exports = {
    ...
    loginRoute: async (req, res, next) => {
        try {
            ...
            return res
                .cookie("token_id", refreshToken, { origin: true, domain: FRONTEND.replace(/https:\/\//, ""), sameSite: "none", httpOnly: true, secure: process.env.NODE_ENV === "production" })
                .cookie("access_token", token, { origin: true, domain: FRONTEND.replace(/https:\/\//, ""), sameSite: "none", httpOnly: true, secure: process.env.NODE_ENV === "production" })
                .status(200)
    ...
    refreshRoute: async (req, res, next) => {
        try {
            ...
            return res
                .cookie("token_id", tokenId, { origin: true, httpOnly: true, domain: FRONTEND.replace(/https:\/\//, ""), sameSite: "none", secure: process.env.NODE_ENV === "production" })
                .cookie("refreshed_token", token, { origin: true, httpOnly: true, domain: FRONTEND.replace(/https:\/\//, ""), sameSite: "none", secure: process.env.NODE_ENV === "production" })
